...
template <typename T> struct aabb2D
{
  ...
  constexpr aabb2D(T minX, T maxX, T minY, T maxY)
  {
    this->minX = minX;
    this->maxX = maxX;
    this->minY = minY;
  ...
  constexpr aabb2D(T minX, T maxX, T minY, T maxY, T centerX, T centerY)
  {
    this->minX = minX;
    this->maxX = maxX;
    this->minY = minY;
  ...
  constexpr aabb2D(vec2<T> center, vec2<T> dimensions)
  {
  ...
  [[nodiscard]] bool containsPoint(T x, T y)
  {
...
template <typename T> struct aabb3D
{
  ...
  /// Check if this AABB contains a given point
  [[nodiscard]] bool containsPoint(T x, T y, T z)
  {
